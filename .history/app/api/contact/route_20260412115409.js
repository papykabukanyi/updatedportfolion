...
function createTransporter() {
  return nodemailer.createTransport({
    // ─── Server ──────────────────────────────────────────────────
    // Port 587 + STARTTLS — Railway blocks outbound port 465 (SMTPS).
    // secure: false means plain TCP then STARTTLS upgrade via EHLO.
    host: 'smtp.gmail.com',
    port: 587,
    secure: false,

    // ─── CRITICAL: force IPv4 ─────────────────────────────────────
    // Railway blocks outbound IPv6 — without this Node resolves
    // smtp.gmail.com to an AAAA record causing ENETUNREACH / ESOCKET.
    family: 4,
    ...
    // ─── TLS options (applied after STARTTLS upgrade) ─────────────
    tls: {
      minVersion: 'TLSv1.2',
      rejectUnauthorized: true,
    },

    // ─── No pooling — Railway kills idle TCP connections silently,
    //     causing stale-pool ETIMEDOUT on subsequent sends.
    pool: false,
...
// Create a fresh transporter per request — avoids stale connections on Railway
function getTransporter() {
  return createTransporter()
}
function resetTransporter() {
  // no-op — per-request mode, nothing to reset
}
