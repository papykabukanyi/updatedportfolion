...
export default function Contact() {
  ...
  const [crowSubStatus, setCrowSubStatus] = useState('idle') // idle | loading | ok | skip

  const handleChange = (e) => {
    setForm((prev) => ({ ...prev, [e.target.name]: e.target.value }))
  }

  async function handleCrowSubscribe() {
    setCrowSubStatus('loading')
    try {
      const res = await fetch('/api/crow/subscribe', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: form.email, name: form.name, city: 'dripping-springs' }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error)
      if (data.id) localStorage.setItem('crow_subscriber_id', data.id)
      setCrowSubStatus('ok')
    } catch { setCrowSubStatus('ok') } // silently succeed
  }
              ...
              <AnimatePresence mode="wait">
                {status === 'success' ? (
                  ...
                  >
                    ...
                    {/* CROW newsletter prompt */}
                    {crowSubStatus === 'idle' && (
                      <div className="mt-4 bg-purple-950/60 border border-purple-700/40 rounded-xl px-5 py-4 text-center max-w-xs w-full">
                        <p className="text-purple-300 text-xs font-bold uppercase tracking-widest mb-1">🐦 CROW News</p>
                        <p className="text-slate-300 text-sm mb-3">Interested in independent local news from Dripping Springs, TX?</p>
                        <div className="flex gap-2 justify-center">
                          <button onClick={handleCrowSubscribe} disabled={crowSubStatus === 'loading'}
                            className="bg-purple-700 hover:bg-purple-600 text-white text-xs font-bold px-4 py-2 rounded-lg transition-colors">
                            Yes, subscribe me!
                          </button>
                          <button onClick={() => setCrowSubStatus('skip')}
                            className="text-slate-500 hover:text-slate-300 text-xs px-4 py-2 rounded-lg transition-colors">
                            No thanks
                          </button>
                        </div>
                      </div>
                    )}
                    {crowSubStatus === 'loading' && (
                      <p className="text-purple-400 text-sm animate-pulse">Subscribing to CROW...</p>
                    )}
                    {crowSubStatus === 'ok' && (
                      <p className="text-purple-400 text-sm font-bold">✓ You're subscribed to CROW News!</p>
                    )}

                    <button
                      onClick={() => { setStatus('idle'); setCrowSubStatus('idle') }}
