        ...
        <div className="fixed inset-0 z-[9999] bg-[#050508] flex flex-col items-center justify-center">
            ...
            <div
              className={`h-full animate-loading-bar ${
                loading === 'crow'
                  ...
                  ? 'bg-gradient-to-r from-orange-500 via-orange-300 to-orange-500'
          ...
          <div
            className={`w-12 h-12 rounded-full border-4 border-white/10 animate-spin ${
              loading === 'crow' ? 'border-t-purple-400' : loading === 'construction' ? 'border-t-orange-400' : 'border-t-amber-400'
            }`}
          />
          <p className="text-slate-400 text-sm font-mono tracking-widest uppercase mt-6">
            {loading === 'crow' ? 'Loading CROW News…' : loading === 'construction' ? 'Loading Papy C&D…' : "Loading Papi's Store…"}
          </p>
        </div>
      )}
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-5">
            ...
            <button
              onClick={() => navigate('construction', CONSTRUCTION_URL)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-orange-500/30 bg-orange-500/5 text-orange-400 text-xs font-bold hover:bg-orange-500/15 hover:border-orange-500/60 transition-all"
            >
              <FiExternalLink size={11} /> PAPY C&amp;D
            </button>
            <span className="w-px h-4 bg-white/10" />
            <select
              value={lang}
        ...
        {mobileOpen && (
          <div className="md:hidden bg-[#050508]/95 backdrop-blur-md border-b border-white/5 px-6 py-5 space-y-4">
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button
                onClick={() => navigate('construction', CONSTRUCTION_URL)}
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-orange-500/30 bg-orange-500/5 text-orange-400 text-xs font-bold hover:bg-orange-500/15 transition-all"
              >
                <FiExternalLink size={11} /> PAPY C&amp;D
              </button>
            </div>
